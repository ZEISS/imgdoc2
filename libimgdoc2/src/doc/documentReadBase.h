...
protected:
    ...
    /// to determine if a dimension is valid, and the other functor 'func_add_dimension_tablename' is used to give the column name for a dimension.
    ...
    /// \param  func_add_dimension_tablename    A functor which adds the column name for a dimension to the specified string stream.
...
private:
    std::shared_ptr<IDbStatement> CreateQueryMinMaxStatement(const std::vector<imgdoc2::Dimension>& dimensions, const std::function<void(std::ostringstream&, imgdoc2::Dimension)>& func_add_dimension_tablename, const std::string& table_name);
};
