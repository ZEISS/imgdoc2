...
TEST(Read2d, TryReadTileInfoForNonExistentTile)
{
    ...
    EXPECT_THROW(
        reader->ReadTileInfo(1234, &tc, &logical_position_info, nullptr),
        imgdoc2::non_existing_tile_exception);
}

TEST(Read2d, CreateDocumentAddSingleTileReadTileInfoCheckForCorrectness)
{
    ...
    position_info.width = 12;
    position_info.height = 13;
    position_info.pyrLvl = 0;
    tileInfo.pixelWidth = 40;
    tileInfo.pixelHeight = 41;
    tileInfo.pixelType = 3;
    ...
    writer->AddTile(&tc, &position_info, &tileInfo, DataTypes::UNCOMPRESSED_BITMAP, TileDataStorageType::BlobInDatabase, &blobData);
    ...
    LogicalPositionInfo logical_position_info;
    TileBlobInfo tile_base_info;
    reader->ReadTileInfo(indices[0], &tile_coordinate_read, &logical_position_info, &tile_base_info);
    ...
    ASSERT_EQ(tile_base_info.base_info.pixelWidth, 40);
    ASSERT_EQ(tile_base_info.base_info.pixelHeight, 41);
    ASSERT_EQ(tile_base_info.base_info.pixelType, 3);
    ASSERT_EQ(tile_base_info.data_type, DataTypes::UNCOMPRESSED_BITMAP);
    int m_read;
